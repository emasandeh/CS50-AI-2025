...
class CrosswordCreator():
        ...
        for variable, words in self.domains.items():
            ...
            for word in words:
                if len(word) != variable.length:
                    wtr.add(word)
            self.domains[variable] = words.difference(wtr)
    ...
    def revise(self, x, y):
        ...
        """
        r = False
        overlp = self.crossword.overlaps[x, y]
        if overlp:
            v1, v2 = overlp
            xs_to_remove = set()
    ...
    def consistent(self, assignment):
        ...
        """
        for v_x, w_x in assignment.items():
            if v_x.length != len(w_x):
                return False
            for v_y, w_y in assignment.items():
                if v_x != v_y:
                    if w_x == w_y:
                        return False
                    overlap = self.crossword.overlaps[v_x, v_y]
                    if overlap:
                        a, b = overlap
                        if w_x[a] != w_y[b]:
                            return False
